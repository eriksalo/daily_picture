...
const httpApi = new HttpApi(apiStack, 'DailyInsectHttpApi', {
  apiName: 'dailyInsectApi',
  corsPreflight: {
...
const generateIntegration = new HttpLambdaIntegration(
  ...
  backend.generateDailyInsect.resources.lambda,
);
...
const generateLambdaStack = Stack.of(backend.generateDailyInsect.resources.lambda);
