...
const USED_KEY = 'state/used-nerds.json';
...
async function getUsedNerds(bucket: string): Promise<string[]> {
  try {
...
async function recordUsedNerd(bucket: string, name: string): Promise<void> {
  const used = await getUsedNerds(bucket);
...
export const handler = async (event?: APIGatewayProxyEventV2): Promise<void | APIGatewayProxyResultV2> => {
  const bucketName = process.env.DAILY_NERD_IMAGES_BUCKET_NAME!;
  ...
  console.log(`Generating daily nerd for ${dateStr}`);

  try {
    const usedNerds = await getUsedNerds(bucketName);
    console.log(`Found ${usedNerds.length} previously used people`);

    const maxRetries = 3;
    let nerd: NerdSelection | null = null;

    for (let attempt = 0; attempt < maxRetries; attempt++) {
      const selectionResponse = await withRetry(() => ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: getNerdSelectionPrompt(usedNerds),
        config: {
          temperature: 1.0,
          ...
          responseSchema: {
            ...
            required: ['name', 'field', 'era', 'contribution', 'fun_facts', 'overlay_fact', 'image_prompt'],
          },
        },
      }), 'nerd selection');
      ...
      if (usedNerds.includes(candidate.name)) {
        console.log(`Attempt ${attempt + 1}: Gemini picked already-used "${candidate.name}", retrying`);
        continue;
      }
    ...
    if (!nerd) {
      throw new Error('Failed to pick a new person after retries');
    }

    console.log(`Selected: ${nerd.name}`);

    const grayscalePrompt = getGrayscaleImagePrompt(nerd.image_prompt, nerd.name, nerd.overlay_fact);
    ...
    try {
      const colorPrompt = getColorImagePrompt(nerd.image_prompt, nerd.name, nerd.overlay_fact);
    ...
    const metadata = {
      ...
      field: nerd.field,
      era: nerd.era,
      contribution: nerd.contribution,
      fun_facts: nerd.fun_facts,
      overlay_fact: nerd.overlay_fact,
    ...
    await recordUsedNerd(bucketName, nerd.name);
    console.log(`Done: ${nerd.name}`);
    ...
    if (isApiCall) {
      return {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: 'Nerd image generated successfully',
          date: dateStr,
          name: nerd.name,
          field: nerd.field,
        }),
